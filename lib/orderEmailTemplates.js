...
//peptides\lib\orderEmailTemplates.js

const LOGO_URL = "https://www.bio-peptides.com/images/Biologo.png";
...
function safe(v) {
  return v && v !== "undefined" ? String(v).trim() : "";
}
...
function formatAddress(address = {}) {
  ...
  if (safe(address.fullName)) {
    lines.push(`<div style="font-weight:700;color:#0f172a;">${safe(address.fullName)}</div>`);
  }

  if (safe(address.house)) lines.push(safe(address.house));
  if (safe(address.area)) lines.push(safe(address.area));

  const cityState = [safe(address.city), safe(address.state)]
    .filter(Boolean)
    .join(", ");

  const cityLine = [cityState, safe(address.pincode)]
    .filter(Boolean)
    .join(" – ");

  if (cityLine) lines.push(cityLine);
  if (safe(address.country)) lines.push(safe(address.country));
  if (safe(address.phone)) lines.push(`📞 ${safe(address.phone)}`);
...
<div style="text-align:left;">
  ...
  ${formatAddress(address)}
              ...
              <div style="text-align:center;margin:24px 0 8px 0;">
                <a href="https://www.bio-peptides.com/orders"
